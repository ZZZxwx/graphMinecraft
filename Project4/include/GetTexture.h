...
class GetTexture
{
	...
	*/
	static  GLuint load2DTexture(const char* filename, GLint internalFormat = GL_RGBA,
		GLenum picFormat = GL_RGBA, int loadChannels = 4)
	{
		// Step1 创建并绑定纹理对象
		GLuint textureId = 0;
		...
		int channels = 0;
		imageData = stbi_load(filename, &picWidth, &picHeight, &channels, loadChannels);
		//cout << picWidth << " " << picHeight << " " << channels << endl;
		//cout << sizeof(imageData) << endl;
		if (imageData == NULL)
		{
